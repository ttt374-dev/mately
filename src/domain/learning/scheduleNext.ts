...
export function scheduleNext(
  ...
  let interval = record.intervalDays ?? IntervalDays.zero()
  let ease = record.easeFactor ?? EaseFactor.initial()

  if (quality < 2) {
    // 不正解
    interval = IntervalDays.of(1)
  } else {
    if (interval.isZero()) {
      interval = IntervalDays.of(1)
    } else if (interval.isOne()) {
      interval = IntervalDays.of(3)
    } else {
      interval = interval.nextWithEase(ease)
    }
  }

  ease = ease.update(quality)

  const nextReviewedAt = ReviewedAt.from(now, interval)
